...
void pio_configure_1wire() {
    pio_qspi_1wire_tx_init(pio_config.pio, pio_config.sm, pio_config.qspi_1wire_offset, 8,
        // 37.5f,
        1.0f, SCREEN_SCK_PIN, SCREEN_SD0_PIN);
...
void pio_configure_4wire() {
    ...
    pio_qspi_4wire_tx_init(pio_config.pio, pio_config.sm, pio_config.qspi_4wire_offset,
        8,  // 8 bit words
        // 37.5f,
        1.0f, SCREEN_SCK_PIN, SCREEN_PIO_SD_START_PIN);
...
void amoled_reset() {
    ...
    amoled_draw_block(
        amoled.offset_x, amoled.offset_y, AMOLED_ACTIVE_WIDTH, AMOLED_ACTIVE_HEIGHT, colour_map[PW_SCREEN_BLACK]);
